...
function buildReceiptHtml(success: SuccessInfo, orderTypeLabel: string): string {
  const now = new Date();
...
<html>
  <head>
    ...
    <style>
      ...
      h1 { text-align: center; font-size: 22px; letter-spacing: 5px; margin: 0; }
      ...
      .footer { text-align: center; margin-top: 26px; font-size: 12px; color: #777; }
    </style>
  </head>
  <body>
    <h1>CREMA</h1>
    ...
    <div class="footer">Thank you for visiting Crema!</div>
  </body>
</html>`;
}
...
export async function printReceipt(success: SuccessInfo, orderTypeLabel: string): Promise<void> {
  const html = buildReceiptHtml(success, orderTypeLabel);
